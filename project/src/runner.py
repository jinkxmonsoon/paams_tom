    ...
    for i,ctx in enumerate(generate_episode(seed,n_turns,scenario_id)):
        ...
        if variant=='baseline_naive': chosen=baseline_naive(ctx)
            ...
            if not config.policies_enabled:
                chosen='SHORT_REPLY' if (ctx.privacy_context or ctx.in_meeting) else ('PRIVACY_MASK' if (i%7==0) else chosen)
            ...
            if ps.active_policy=='privacy_masking': chosen=SAFE_ACTION
        ...
        rec={"turn_id":i,"scenario_id":scenario_id,"seed":seed,"context_signals":ctx.to_dict(),"gold_action":gold,"chosen_action":chosen,
             ...
             "top10":top10,"policy_switch_count":ps.switch_count,"policy_required":(ctx.privacy_context or ctx.in_meeting),"safety_action_taken":(chosen==SAFE_ACTION)}
        rows.append(rec)
